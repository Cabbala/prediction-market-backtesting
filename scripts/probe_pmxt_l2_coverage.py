...
async def probe_candidate(
    ...
    *,
    ...
    if caught_gap_warning is not None:
        try:
            gap_hours_missing = int(caught_gap_warning.split("PMXT:", 1)[1].split("archive", 1)[0].strip())
...
def _build_pass_manifest(summary: dict[str, Any]) -> dict[str, Any]:
    pass_results = [
        CoverageProbeResult(**row) for row in summary["results"] if row.get("status") == "pass"
    ]
    return {
        "schema_version": 1,
...
def _write_markdown(summary: dict[str, Any], md_path: Path, pass_manifest_path: Path) -> None:
    lines = [
        ...
        "## Candidates",
        "",
        "| status | market | token | book_events | message |",
        "|---|---|---:|---:|---|",
    ]
    for row in summary["results"]:
        lines.append(
